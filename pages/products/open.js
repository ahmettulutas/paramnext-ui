...
export async function getStaticProps () {
  return {
    props: {
      pageInfo: await getContentfulPage("productsOpenBanking")
    }
  };
}

const
  productName = productNames.open,
  Open = ({ pageInfo }) => {
    const
      intl = useIntl(),
      pageTitle = intl.formatMessage({ id: `label.product.${productName}` }),
      title = `ParamTech | ${pageTitle}`,
      { heroTitle, heroSubtitle, heroDescription, pageMetaDescription, productTitle, productDescription, solutionsImage, solutions } = pageInfo.items[0].fields;
